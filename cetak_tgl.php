...
$sheet->setCellValue('Q3', 'HbA1C');
$sheet->setCellValue('R3', 'TRIGLISERIDA');
$sheet->setCellValue('S3', 'FUNGSI GINJAL');
$sheet->setCellValue('T3', 'RENCANA PEMERIKSAAN LAB ULANGAN ');
...
while($d = mysqli_fetch_array($data))
{
    ...
    $sheet->setCellValue('B'.$i, $d['nama_p']);
    $sheet->setCellValue('C'.$i, $d['bpjs_p']);
    $sheet->setCellValue('D'.$i, $d['nik_p']);
    $sheet->setCellValue('E'.$i, $d['tgl_lahir_p']);    
    $sheet->setCellValue('F'.$i, $d['alamat_p']);
    ...
    $sheet->setCellValue('Q'.$i, $d['hba1c']); 
    $sheet->setCellValue('R'.$i, $d['trigliserida']);
    $sheet->setCellValue('S'.$i, $d['f_ginjal']);
    if($d['p_ulang'] =='0000-00-00'){
    ...
    if($d['bmi'] >= '30' || $d['t_darah'] >= '140/80'){
        ...
        $sheet->setCellValue('U'.$i, 'Terkontrol'); 
    
    }
    $sheet->setCellValue('V'.$i, $d['keterangan']);     
    
if($d['sts1']=='0' && strtotime($d['p_ulang']) <= strtotime($t)){
    ...
    $spreadsheet->getActiveSheet()->getStyle('V'.$i)->applyFromArray([
        'font' => [
            'color' => ['argb' => 'FFFF0000']
        ],
        'alignment' => [
